...
async function main() {
  ...
  console.log(`📊 Found ${brokenBookings.length} bookings with paidAmount = 0 but status = PAID\n`);
  ...
  for (const booking of brokenBookings) {
    ...
    fixedCount++;
    console.log(`  ✅ Fixed #${booking.bookingCode}: paidAmount = ${Number(booking.totalPrice).toLocaleString()}đ`);
  }
  ...
  if (stillBroken === 0) {
    console.log('✅ Verification passed: All paid bookings now have correct paidAmount');
  } else {
    console.log(`⚠️ Warning: ${stillBroken} bookings still have paidAmount = 0`);
  }
}
