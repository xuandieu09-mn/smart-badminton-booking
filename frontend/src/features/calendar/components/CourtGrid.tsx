...
export const CourtGrid: React.FC<Props> = ({ courts, isLoading, selectedCourtId, onSelect }) => {
	if (isLoading) {
		return <p className="text-gray-500">Đang tải sân...</p>;
	}

	if (!courts || courts.length === 0) {
		return <p className="text-gray-500">Chưa có sân khả dụng. Vui lòng tạo sân hoặc thử lại sau.</p>;
	}
					...
					>
						<div className="flex items-center justify-between">
							<div>
								...
								{court.description && (
									<div className="text-sm text-gray-600 line-clamp-2">{court.description}</div>
								)}
							</div>
