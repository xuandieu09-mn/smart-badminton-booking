...
export const SocketProvider: React.FC<SocketProviderProps> = ({ children }) => {
  ...
  const connect = () => {
    const token = localStorage.getItem('access_token'); // Must match API client
    if (!token) {
      console.warn('❌ No token found, cannot connect to socket');
      return;
    }

    console.log('🔌 [DEBUG] Connecting with token:', token.substring(0, 20) + '...');
    ...
    // 🔍 DEBUG: Catch ALL events from server
    ...
    newSocket.on('connected', (data: any) => {
      ...
      console.log('🔍 [DEBUG] User role from server:', data.role);
